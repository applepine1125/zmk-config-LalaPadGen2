...
- パッド行は画面幅を 2 分割し、左半分に左右のパッド(縦横比は解像度 X 2457 × Y 3072)を横に並べ、右半分を「検知した操作」パネルにする。パッドは `T F` 行(指の本数、各指の座標、フラグ、hold、2F モード)から指の位置を点で描き、状態で色を変える(待機 / 1 本指 / 2 本指スクロール / 2 本指ピンチ / 3 本指 / ドラッグ中 / ボタン押下)。操作パネルは側ごとの「今の操作」(直近の `T F` と K イベントから)と、検知した操作の流れ(要約の認識文とクリック・スクロールの開始を時刻・側つきで最新順に 30 件)を出す。`T F` が来ない側は「ライブ表示なし(要約のみ)」と示す。見た目は CSS 変数で統一したモダンなトーン(淡い背景、角丈のカード、アクセント 1 色、ダークモード追従、絵文字なし)
...
- 送信は `tp live` の状態と `stream` の購読でゲートし、アプリが切断したら止める。レートは固定せずランタイム設定にする(`tp live on|off [回/秒]`、既定は右 60・左 30。ドライバのフレーム約 100 回/秒が上限)。Mac との BLE 接続間隔(約 15ms)は 1 イベントに複数パケット送れるので上限の壁ではなく、実際の制約はマウス HID レポートと split リンクとの取り合いなので、実機でカーソルやキーの遅れが出ない上限を体感で決める
- 左手の試行要約(1 試行 10 通知)は現状アプリ未接続でも右手へ送っている。5b では右手が `stream` の購読開始/終了に合わせて左手へ `summary on|off` を指示し、アプリがないときは何も送らないようにする
