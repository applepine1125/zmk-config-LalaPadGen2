...
- peripheral → central: `zmk_split_peripheral_report_event` の入力イベントを `reg = 2` で送る。central 側は右手オーバーレイの `zmk,input-split` ノード `tp_tuner_split_L@2`(`reg = <2>`、`zmk,input-listener` は付けない)で受け、`INPUT_CALLBACK_DEFINE` で自前のハンドラに渡す。イベント種別は `type` で区別する:
...
- peripheral 側の送信は work で 1 イベントずつ行い、`-ENOMEM`/`-EAGAIN`/`-ENOBUFS` なら 10ms 後に同じ index から再試行する(最大 50 回)。要約はキュー(深さ 4)に積む。1 試行あたり 10 通知、dump は 55 通知で、いずれもパッドが触られていない間に流れる
- central 側は左手向けコマンドを 1 つずつ処理する(処理中は次を待たせる)。`L set <name> <value>` は名前を index に解決し範囲を central でも検証してから送り、ACK を `L OK <name>=<value>` / `L ERR out of range <min>..<max>` に整形する。`L list` は dump を送り、PARAM を `L <name> <value> <min> <max> <kind> <default>`(名前・範囲・既定値は central 側の同じテーブルから)に整形し、STATUS で終端する。`L info` は STATUS を `L side=peripheral uptime_ms=<n> params=<n> saved=<yes|no>` に整形する。`L reset`/`reati`/`save`/`summary on|off` は ACK を `L OK …`/`L ERR <errno>` にする。`L get`/`L trace` は `L ERR unsupported`。1000ms 応答がなければ `L ERR timeout`。いずれも最後に `L .`
